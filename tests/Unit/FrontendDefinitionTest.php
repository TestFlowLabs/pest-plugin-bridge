...
describe('FrontendDefinition', function (): void {
    ...
    describe('trustExistingServer', function (): void {
        test('defaults to false', function (): void {
            $definition = new FrontendDefinition('http://localhost:3000');

            expect($definition->shouldTrustExistingServer())->toBeFalse();
        });

        test('can be enabled', function (): void {
            $definition = new FrontendDefinition('http://localhost:3000');
            $definition->trustExistingServer();

            expect($definition->shouldTrustExistingServer())->toBeTrue();
        });

        test('returns self for fluent chaining', function (): void {
            $definition = new FrontendDefinition('http://localhost:3000');
            $result     = $definition->trustExistingServer();
